...
- [x] 1. Lane init: `PROGRESS.md` + `DECISIONS.md` + `feature_list.json` with one row per SPEC §4
...
- [x] 2. Cold baseline of `designing-consistently`: run it as it stands today against
...
- [x] 3. Cold baseline of `extracting-design-md`: same method, its 7 steps, into
...
- [x] 4. Reconcile the baselines against SPEC §5's three recorded predictions (step 1 offers to
...
- [x] 5. Evals for `designing-consistently`, written from the step-2 baseline and the step-4
...
- [x] 6. Evals for `extracting-design-md`, from the step-3 baseline and the step-4 rulings: 2 new
...
- [x] 7. Rewrite `skills/designing-consistently/SKILL.md` to pass the 5 evals from step 5:
...
- [x] 8. Rewrite `skills/extracting-design-md/SKILL.md` to pass the 6 evals from step 6:
...
- [x] 9. `[batch]` Sweep live stale references and refresh the two skill rows in `README.md` to
...
- [x] 10. Write a DESIGN.md for the admin to the approved shape into
...
- [x] 11. `work-verify` pass over SPEC §4 requirements 1-11; every `feature_list.json` row moves
